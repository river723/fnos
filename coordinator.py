            ...
            for item in output.split(';'):
                if ':' in item:
                    ...
                    try:
                        if key == 'DISK_INFO':
                            import json
                            disk_info = json.loads(value.strip())
                            data['disk_info'] = disk_info['blockdevices']
                        elif key == 'DISK_IO':
                            ...
                            temp = value.strip()
                            try:
                                data['cpu_temperature'] = float(temp)
                            ...
                            if self._last_net_stats and self._last_update_time:
                                interval = now - self._last_update_time
                                data['download_speed'] = round((rx_bytes - self._last_net_stats[0]) * 512 / interval / 1_000_000, 2)
                                data['upload_speed'] = round((tx_bytes - self._last_net_stats[1]) * 512 / interval / 1_000_000, 2)
